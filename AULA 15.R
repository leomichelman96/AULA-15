...
iota <- na.omit(read_excel("C:/Econometria/iota"))

iota <-  ts(iota$Close, start = 2014, frequency = 365)

plot(iota)

#Verificar se a Série é Estacionária - Não é ESTACIONÁRIA

#Criar FAC  e FACP

acf(iota$Close,lend=2, lwd=5,col="darkblue",main= "Função Autocorrelação - FAC")              #Melhorando aspecto da FAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

pacf(iota$Close,lend=60, lwd=5,col="darkblue",main= "Função Autocorrelação Parcial - FACP")   #Melhorando aspecto da PAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")
